...
public class AuthInterceptor extends HandlerInterceptorAdapter{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception{
		...
		if(System.currentTimeMillis() > hashCodeOptional.get().getExDate().getTime() && hashCodeOptional.get().getType().equals("Login")){
			response.setStatus(401);
			return false;
		}
		...
		if(annotation != null) {
			...
			for(String permission : annotation.permissions())
				if(!permissionFilter.filter(user.getId(), Integer.valueOf(pathVariables.get("id")), permission)) {
					response.sendError(403,"You do not have permission for this request.");
					return false;
				}
		}
		return true;
